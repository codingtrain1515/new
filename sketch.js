const Engine = Matter.Engine;
const Render = Matter.Render;
const World = Matter.World;
const Bodies = Matter.Bodies;
const Constraint = Matter.Constraint;
const Body = Matter.Body;
const Composites = Matter.Composites;
const Composite = Matter.Composite;

let engine;
let world;
var rope,fruit,ground;
var fruit_con;
var fruit_con_2;

var bg_img;
var food;
var rabbit;

var button,button2,blower;
var bunny;
var blink,eat,sad;
var mute_btn;

var fr,rope2;



var bk_song;
var cut_sound;
var sad_sound;
var eating_sound;
var air;
var blowerbtn;
var muteBtn;
var bubble;
var bubImg;
var shelf;


function preload()
{
  bg_img = loadImage('background.png');
  food = loadImage('melon.png');
  rabbit = loadImage('Rabbit-01.png');

  bk_song = loadSound('sound1.mp3');
  sad_sound = loadSound("sad.wav")
  cut_sound = loadSound('rope_cut.mp3');
  eating_sound = loadSound('eating_sound.mp3');
  air = loadSound('air.wav');

  blink = loadAnimation("blink_1.png","blink_2.png","blink_3.png");
  eat = loadAnimation("eat_0.png" , "eat_1.png","eat_2.png","eat_3.png","eat_4.png");
  sad = loadAnimation("sad_1.png","sad_2.png","sad_3.png");
  bubImg =loadImage('bubble.png');
  
  blink.playing = true;
  eat.playing = true;
  sad.playing = true;
  sad.looping= false;
  eat.looping = false; 
}

function setup() {
  createCanvas(500,700);

  frameRate(80);

  bk_song.play();
  bk_song.setVolume(0.5);

  engine = Engine.create();
  world = engine.world;
  //ROPE2
  button2= createImg('cut_btn.png');
  button2.position(15,310);
  button2.size(50,50);
  button2.mouseClicked(drop2);

  button = createImg('cut_btn.png');
  button.position(205,250);
  button.size(50,50);
  button.mouseClicked(drop);





muteBtn = createImg("mute.png");
muteBtn.position(450,50);
muteBtn.size(50,50);
muteBtn.mouseClicked(muteBackground)

  rope = new Rope(4,{x:245,y:250});
  ground = new Ground(200,690,600,20);


  

  blink.frameDelay = 20;
  eat.frameDelay = 20;

  bunny = createSprite(300,100,100,100);
  bunny.scale = 0.2;

  bunny.addAnimation('blinking',blink);
  bunny.addAnimation('eating',eat);
  bunny.addAnimation('crying',sad);
  bunny.changeAnimation('blinking');

  var fruitOptions = {
    restitution:0.8
  }

  fruit = Bodies.circle(300,300,20,fruitOptions);
  Matter.Composite.add(rope.body,fruit);
  

  fruit_con = new Link(rope,fruit);


  rope2 = new Rope(4,{x:15,y:310});
  
  //fruit_con_2 = new Link(rope2,fruit);

  bubble =createSprite(230,520,20,20);
  bubble.addImage(bubImg);
  bubble.scale =0.1;

  shelf =new Ground(300,170,100,20)

  rectMode(CENTER);
  ellipseMode(RADIUS);
  textSize(50)
  
}

function draw() 
{
  background(51);
  image(bg_img,0,0,490,690);
 
  push();
  imageMode(CENTER);
  if(fruit!=null){
    image(food,fruit.position.x,fruit.position.y,70,70);
  }
  pop();

  rope.show();
  rope2.show();
  Engine.update(engine);
  ground.show();
  shelf.show();

  drawSprites();

  if(collide(fruit,bunny,80)==true)
  {
    bunny.changeAnimation('eating');
    eating_sound.play();
  }


  if(fruit!=null && fruit.position.y>=850)
  {
    bunny.changeAnimation('crying');
    sad_sound.play();
     //fruit=null;
     
   }

   if(collide(fruit,bubble,40)==true){

    engine.world.gravity.y=-1;
    //bubble.position.x = fruit.position.x;
    //bubble.position.y = fruit.position.y;

   }
   
}

function drop()
{
  
  rope.break();
  fruit_con.detach();
  fruit_con = null; 
}

function drop2(){

rope2.break();
//fruit_con2.detach();
//fruit_con2 = null; 
}

function collide(body,sprite,x)
{
  if(body!=null)
        {
         var d = dist(body.position.x,body.position.y,sprite.position.x,sprite.position.y);
         console.log(d)
          if(d<=x)
            {
              World.remove(engine.world,fruit);
               fruit = null;
               return true; 
            }
            else{
              return false;
            }
         }
}



function keyPressed(){
  if(keyCode=== 32){
    airBlow()
  }
}

function muteBackground(){
  if(bk_song.isPlaying()){
    bk_song.stop();
  }else{
    bk_song.play();
  }
}